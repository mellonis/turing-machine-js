...
**Reading runtime sequence on tape `['a','b','X','b','a']`:** enter the `halt frame` at `[[scanToX]]` (with `eraseHere` on the stack); `* → ·/R` self-loops until the head sees `X`; the `X → ·/S` solid edge would normally halt — it lands on the cloned halt `c3`, the wrapper's catch-and-redirect kicks in, pop the stack → `eraseHere`; `eraseHere` runs `* → ⌫/S` and halts at real `s0`. Run terminates.
