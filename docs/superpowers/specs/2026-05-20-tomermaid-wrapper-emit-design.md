...
2. A **halt-marker `(((halt)))` node inside that subgraph** — visualization of "halt-bound transitions land here, *inside* the wrapper's scope." `haltState` is a runtime singleton; the halt marker is a teaching aid (one per wrapper context on the diagram, all corresponding to the single runtime instance).
