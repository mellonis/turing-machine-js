...
class Tape {
  constructor({
    ...
    if (tapeSymbolList.length === 0) {
      if (!alphabetSymbolList) {
        ...
        tapeSymbolList.push(_alphabet2.blankSymbol);
      }
    } else {
      if (!alphabetSymbolList) {
        _alphabet2 = new Alphabet(tapeSymbolList);
      } else {
        _alphabet2 = new Alphabet(alphabetSymbolList);
        const isValidTape = tapeSymbolList.every(symbol => _alphabet2.has(symbol));
      ...
      tapeSymbolList = tapeSymbolList.map(symbol => _alphabet2.index(symbol));
    }

    _classPrivateFieldSet(this, _alphabet, _alphabet2);

    _classPrivateFieldSet(this, _tape, tapeSymbolList);
  ...
  set symbol(symbol) {
    ...
    _classPrivateFieldGet(this, _tape)[_classPrivateFieldGet(this, _position)] = _classPrivateFieldGet(this, _alphabet).index(symbol);
  }

  get symbol() {
    return _classPrivateFieldGet(this, _alphabet).get(_classPrivateFieldGet(this, _tape)[_classPrivateFieldGet(this, _position)]);
  }

  get symbolList() {
    return _classPrivateFieldGet(this, _tape).slice(_classPrivateFieldGet(this, _position) - this.extraCellsCount, _classPrivateFieldGet(this, _position) + this.extraCellsCount + 1).map(index => _classPrivateFieldGet(this, _alphabet).get(index));
  }
  ...
  normalise() {
    while (_classPrivateFieldGet(this, _position) - this.extraCellsCount < 0) {
      _classPrivateFieldGet(this, _tape).unshift(0);

      _classPrivateFieldSet(this, _position, _classPrivateFieldGet(this, _position) + 1);
    }

    while (_classPrivateFieldGet(this, _position) + this.extraCellsCount >= _classPrivateFieldGet(this, _tape).length) {
      _classPrivateFieldGet(this, _tape).push(0);
    }
  }
...
class TuringMachine {
  constructor(_tape3 = null) {
    ...
    _classPrivateFieldSet(this, _tape2, _tape3);
  }
  ...
  run(initialState, stepsLimit = 1e5) {
    ...
    while (!state.isHalt) {
      ...
      const currentSymbol = _classPrivateFieldGet(this, _tape2).symbol;
      ...
      switch (command.symbol) {
        case symbolCommands.erase:
          nextSymbol = _classPrivateFieldGet(this, _tape2).alphabet.blankSymbol;
          break;
      ...
      _classPrivateFieldGet(this, _tape2).symbol = nextSymbol;

      switch (nextMovement) {
        case movements.left:
          _classPrivateFieldGet(this, _tape2).left();

          break;

        case movements.right:
          _classPrivateFieldGet(this, _tape2).right();
